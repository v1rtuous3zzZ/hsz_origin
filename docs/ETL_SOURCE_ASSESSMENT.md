...
- 调用 `sp_create_event_month_tables('202608')` 后，新 ODS 月表为 `trade_id varchar(38) ascii_bin NOT NULL` 且有 `uk_ods_trade_id`；新命中月表为 `trade_id varchar(38) ascii_bin NOT NULL` 且有 `uk_event_object_match (trade_id, object_no)`。
