"""凌晨核对最近窗口，只补源端唯一交易数大于中心入库数的服务器窗口。"""
...
from app.etl.formal_sync import sync_window  # noqa: E402
from app.etl.source_config import load_mapping, load_sources  # noqa: E402
from app.etl.source_reader import (  # noqa: E402
    realtime_complete_windows,
    source_connection,
    window_counts,
)
from app.etl.source_schema import monthly_table  # noqa: E402
...
def latest_source_counts(
    ...
    """状态取最近批次，数量直接取中心 ODS 唯一事件，兼容旧批次计数口径。"""
    rows = db.execute(
        text(
...
def reconcile(
    ...
    for source in sources:
        ...
        try:
            ...
            complete_windows = [window for window in windows if window[0] in complete_realtime]
            for range_start, range_end in _contiguous_ranges(complete_windows):
            ...
            for range_start, range_end in _contiguous_ranges(incomplete_windows):
